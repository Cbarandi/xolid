...
export function WaitlistAccessModal({ open, onClose }: Props) {
  const titleId = useId();
  ...
  return (
    ...
    >
      ...
      >
        ...
        <h2 id={titleId} className="mt-3 text-[26px] font-medium leading-[1.05] tracking-[-0.04em] text-white sm:text-[30px]">
          Private waitlist
        </h2>
        <p className="mt-4 text-[14px] leading-7 text-white/52 sm:text-[15px]">
          Join selectively. We open the system in waves.
        </p>

        <WaitlistForm submitLabel="JOIN THE WAITLIST" className="mt-6" />
      </div>
    </div>
  );
}
